...
describe('login transaction security regressions', () => {
  test('state is hashed, provider-bound, return-bound, and consumed once', async () => {
    ...
    const response = await api.fetch(req(`/api/auth/callback/generic?state=${state}`));
    expect(response.status).toBe(302); expect(response.headers.get('location')).toBe('/owned');
    expect(response.headers.getSetCookie()).toHaveLength(2);
    expect((await api.fetch(req(`/api/auth/callback/generic?state=${state}`))).status).toBe(400);
    expect(api.database.query('SELECT count(*) count FROM sessions').get().count).toBe(1);
  });
  test('form_post succeeds without state cookie and expiry fails', async () => {
    let now = NOW; const api = backend({ clock: () => now }); const state = await login(api);
    const post = await api.fetch(req('/api/auth/callback/generic', { method: 'POST', headers: { 'content-type': 'application/x-www-form-urlencoded' }, body: `state=${state}` }));
    expect(post.status).toBe(302);
    const expired = await login(api); now += LOGIN_STATE_DURATION_MS + 1;
    expect((await api.fetch(req(`/api/auth/callback/generic?state=${expired}`))).status).toBe(400);
  });
  ...
  test('secret changes hashes and invalidates existing state and session material', async () => {
    ...
    expect((await apiRotated.fetch(req(`/api/auth/callback/generic?state=${state}`))).status).toBe(400);
  ...
  test('competing callbacks deterministically create one identity and replay loses', async () => {
    ...
    const results = await Promise.all([api.fetch(req(`/api/auth/callback/generic?state=${first}`)), api.fetch(req(`/api/auth/callback/generic?state=${second}`))]);
    ...
    const replayResults = await Promise.all([api.fetch(req(`/api/auth/callback/generic?state=${first}`)), api.fetch(req(`/api/auth/callback/generic?state=${first}`))]);
  ...
  test('callback failure rolls back consumption, identity, revocation, and replacement session', async () => {
    ...
    expect((await api.fetch(req(`/api/auth/callback/generic?state=${state}`, { headers: { cookie: `${SESSION_COOKIE_NAME}=${old.raw}` } }))).status).toBe(500);
